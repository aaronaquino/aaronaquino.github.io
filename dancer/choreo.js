$(document).ready(function () {
	$('#tmd-iframe').on('load', function () {
		$('#tmd-spinner').hide();
	});
	$('#wildfire-iframe').on('load', function () {
		$('#wildfire-spinner').hide();
	});
	$('#flex-iframe').on('load', function () {
		$('#flex-spinner').hide();
	});
	$('#liftoff-iframe').on('load', function () {
		$('#liftoff-spinner').hide();
	});
	$('#21-iframe').on('load', function () {
		$('#21-spinner').hide();
	});
	$('#fall-iframe').on('load', function () {
		$('#fall-spinner').hide();
	});
	$('#closer-iframe').on('load', function () {
		$('#closer-spinner').hide();
	});
	$('#baby-iframe').on('load', function () {
		$('#baby-spinner').hide();
	});
	$('#photograph-iframe').on('load', function () {
		$('#photograph-spinner').hide();
	});
	$('#fym-iframe').on('load', function () {
		$('#fym-spinner').hide();
	});
});